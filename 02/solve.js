...
const solve = f => fs.readFileSync(f,"utf8").split(",").map(e=>e.split("-").map(Number)).reduce(({a,b},[s,e])=>{for(let n=s,t;n<=e;n++)t=n+"",/^(\d+)\1$/.test(t)&&(a+=n),/^(\d+)\1+$/.test(t)&&(b+=n);return {a,b}},{a:0,b:0});
